"""Tests for runtime reloading of Argo CD connections.yaml."""
...
async def list_connections(server: ReadOnlyArgoCDServer) -> list[dict[str, object]]:
...
async def test_tools_reload_connections_after_config_changes(tmp_path, monkeypatch):
    monkeypatch.setattr(server_module, "ArgoCDConnector", ReloadTestConnector)
    ...
    server = ReadOnlyArgoCDServer(runtime_paths)
        ...
        [
            {
                ...
                "timeout": 60,
                "verify_ssl": False,
                "session_token": "beta-token-v2",
            },
            {
                "connection_name": "gamma",
                "url": "https://gamma.example.com",
                "description": "Gamma",
                "session_token": "gamma-token",
            },
        ],
    )
    ...
    with pytest.raises(ToolError, match="Connection 'alpha' not found"):
        await call_tool(server, "get_version", {"connection_name": "alpha"})

    beta_version = await call_tool(server, "get_version", {"connection_name": "beta"})
    assert json.loads(beta_version) == {
        "name": "beta",
        "url": "https://beta-v2.example.com",
        "description": "Beta Reloaded",
        "timeout": 60,
...
@pytest.mark.asyncio
async def test_reload_updates_reused_connector_session_token(tmp_path, monkeypatch):
    monkeypatch.setattr(server_module, "ArgoCDConnector", ReloadTestConnector)
    runtime_paths = make_runtime_paths(tmp_path)
    write_connections_file(
        runtime_paths.connections_file,
        ...
        [
            {
                ...
                "session_token": "alpha-token-reloaded",
            }
        ],
    )

    alpha_version = await call_tool(server, "get_version", {"connection_name": "alpha"})
    assert server._connectors["alpha"] is alpha_connector
    assert json.loads(alpha_version) == {
        "name": "alpha",
        "url": "https://alpha.example.com",
        "description": "Alpha Reloaded",
        "timeout": 30,
...
async def test_invalid_reload_keeps_last_good_connections_until_file_changes(
    ...
    alpha_version = await call_tool(server, "get_version", {"connection_name": "alpha"})
    assert json.loads(alpha_version)["url"] == "https://alpha.example.com"
    ...
    beta_version = await call_tool(server, "get_version", {"connection_name": "beta"})
    assert json.loads(beta_version)["url"] == "https://beta.example.com"
...
async def test_failed_reload_does_not_partially_mutate_reused_connectors(
    ...
    monkeypatch.setattr(server_module, "ArgoCDConnector", FailingReloadTestConnector)
    runtime_paths = make_runtime_paths(tmp_path)
    write_connections_file(
        runtime_paths.connections_file,
        [
